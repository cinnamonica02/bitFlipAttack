...
class UmupBitFlipAttack:
    """
    Implementation of the Unit Microparameter Bit Flipping Attack (UmupBFA) on deep neural networks.
    This attack is specifically designed for models using u-μP quantization, identifying and
    flipping critical bits in model parameters to induce misclassification with minimal changes.
    """
    ...
    def perform_attack(self, target_class=0, num_candidates=1000, population_size=50, 
        ...
        for idx in best_solution:
            ...
            if param_idx >= num_elements:
                print(f"Warning (in perform_attack - Umup): Correcting param_idx {param_idx} before calculating coords for shape {shape_for_coords}.")
